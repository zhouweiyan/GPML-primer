...
ObjV=objfunc(hyp_popu,NIND,X_train,y_train,X_test,f_test,ns);   %计算目标函数值
while gen<MAXGEN
   fprintf('%d\n',gen)
   ...
   ObjVSel=objfunc(hyp_popu,size(hyp_popu,1),X_train,y_train,X_test,f_test,ns);             %计算子代的目标函数值
   ...
   [Y,I]=min(ObjV);
   trace(1:N,gen)=hyp_popu(I,:);                       %记下每代的最优值
...
%% define functions
function ObjV=objfunc(hyp_popu,NIND,X_train,y_train,X_test,f_test)
% 用来分别求解种群中各个个体的目标值
% 输入
