...
%         cpa={'covPoly','ard',[],3};
%         c=2;sf=1;L=[1;1];cov=cpa;hyp=log([L;c;sf]); %k(x,z) = sf^2*(c+s)^d , where s=x*inv(P)*z
    case 6
        cm={'covMaterniso',3}; 
        cov=cm;hyp=[];
    case 7
...
%         L=[0.9;0.9];sf=2;al=1;hypra=log([L;sf;al]);cov=cra;hyp=hypra;
    case 8
%         cgu={'covSEisoU'};  % k(x,z) = exp(-(x-z)'*inv(P)*(x-z)/2); P=ell^2*I;
%         ell=0.5;hypgu=log(ell);cov=cgu;hyp=hypgu;
