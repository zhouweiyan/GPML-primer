...
switch opt
    ...
    case 7
        ...
        mean = m1;  hyp = hyp1;
    case 8
        mp = {@meanPoly,3}; hypp = [1;1;2;3;1;1];   % m(x) = x1+x2+2*x1^2+3*x2^2+x1^3+x2^3;
        mean = mp;  hyp = hypp;                 % x(:,1)+x(:,2)+2*x(:,1).^2+3*x(:,2).^2+x(:,1).^3+x(:,2).^3
    case 9
    ...
    case 11 % Sum
        ...
        % x(:,1)+x(:,2)+2*x(:,1).^2+3*x(:,2).^2+x(:,1).^3+x(:,2).^3+2*x(:,1)+3*x(:,2)
        % mp = {@meanPoly,3}; hypp = [3;4;2;3;1;1]; mean = mp; hyp = hypp;
        mean={'meanSum',{ml,mp}};hyp=[hypl;hypp];   
    case 12 % Prod
        mc = {@meanConst};  hypc = 2;
        ml = {@meanLinear}; hypl = [0.2;0.3];
        mpr = {@meanProd,{mc,ml}};     hyppr = [hypc; hypl];      % product
        mean = mpr; hyp = hyppr;    % (0.2*x(:,1)+0.3*x(:,2))*2
    case 13 % meanPow
        mc = {@meanConst};  hypc = 2;
        ml = {@meanLinear}; hypl = [2;3]; 
        msu = {'meanSum',{mc,ml}};      hypsu = [hypc; hypl];   % sum
        mpo = {'meanPow',3,msu};        hyppo = hypsu;          % third power
        mean = mpo; hyp = hyppo;    % (2*x(:,1)+3*x(:,2)+2).^3
    case 14 % meanMask
        ml = {@meanLinear}; hypl = [2;3];
        mask = [false,true];        % mask excluding all but the 2nd component
        mma = {'meanMask',mask,ml};    hypma = hypl(mask);
        mean = mma; hyp = hypma;    % 3*x(:,2)
    case 15 % meanPref
        ml = {@meanLinear}; 
        mpf = {@meanPref,ml};          hyppf = 2;  % linear pref with slope
        mean = mpf; hyp = hyppf;    % 2*x(:,1)-2*x(:,2)
    case 16
        ml = {@meanLinear};
        mwp = {@meanWarp,ml,@sin,@cos};
        hypwp = [2;1]; % sin of linear, [m,dm] = meanWarp(mean, g, dg, hyp, x)
        mean = mwp; hyp = hypwp;    % sin(2*x(:,1)+x(:,2))
end

%% plot and verify the calculation
