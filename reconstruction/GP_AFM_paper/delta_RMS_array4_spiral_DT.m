...
RMS = zeros(1,length(num_curves));
% for i = 1:length(num_curves)
%%
for i = 1:length(num_curves)
    i
    load('Copy_of_array4_GP.mat')
%     opt='GP';
    opt='DT';
    ...
    RMS(i) = sqrt(1/(size(y_test_ideal_a,1)*size(y_test_ideal_a,2))*sum((m_a(:)-y_test_ideal_a(:)).^2))
end
fileID = fopen('E:\OneDrive - hnu.edu.cn\tools\matlabcourse\GPML_matlab\GPML-primer\reconstruction\GP_AFM_paper\delta_PSNR\RMS_array4_spiral_DT.txt','a');
fprintf(fileID, '%8f\n', RMS);
fclose(fileID);