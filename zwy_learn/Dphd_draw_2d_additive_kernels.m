...
% Figure 1, rationale of covADD, equation 1~5
% zhouweiyan 20180724 20180914
...
[a,b]=meshgrid(range);
xstar=[a(:) b(:)];
set(0,'DefaultFigureWindowStyle','docked')  % keep figures natty
%% plot a SE kernel
covfunc='covSEiso';
hyp.cov=log([1;1]);
...
% y=mvnrnd(zeros(length(xstar),1),K,1)';
figure(2)
...
% colormap(jet)
figure
y=chol(K1+K2-eye(n)*.000001,'lower')*gpml_randn(rand,n,1);
surf(a,b,reshape(y,length(range),length(range)),'EdgeColor','none',...
'LineStyle','none','FaceLighting','phong');
colormap(jet)
title('additive kernel');
